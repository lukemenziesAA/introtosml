...
ratio = float(#Fill in here)
sampled_majority_df = major_df.sample(False, #Fill in here)
...
model = LogisticRegression(
    featuresCol=outputCols, labelCol=label + "_num", maxIter=10
)
...
transformer = ColumnTransformer(
    [("cat", #Fill in here, cat_data)], remainder="passthrough"
)
...
pipeline_u.fit(X_train, y_train)
pipeline.fit(X_train, y_train)
prediction_u = pipeline_u.predict(X_test)
...
classes = y.unique()
results_u = classification_report(
    y_test, prediction_u, labels=y.unique(), output_dict=True
)
results = classification_report(y_test, prediction, labels=y.unique(), output_dict=True)

# COMMAND ----------

pd.DataFrame(results_u)
