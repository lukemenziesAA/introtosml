...
# MAGIC </div>
# MAGIC
# MAGIC # *MLFLow Demo*. Presented by <a href="www.advancinganalytics.co.uk">Advancing Analytics</a>
...
# from mlflow.sklearn import eval_and_log_metrics ##Please comment this out
...
try:
    experiment_id = mlflow.create_experiment(
        "/Users/luke@advancinganalytics.co.uk/MLFlow_Experiment"
    )
except:
...
e_set = mlflow.set_experiment(
    experiment_name="/Users/luke@advancinganalytics.co.uk/MLFlow_Experiment"
)
...
# Initialise model - let's use another model
...
trans = ColumnTransformer([("scale", StandardScaler(), X.columns)])
...
#     metrics = eval_and_log_metrics(model, X_test_trans, y_test, prefix="val_") ## Commen this out
#     mlflow.log_metrics(metrics)
    mlflow.log_artifact("scaling_params.json")
...
# client = MlflowClient()
# outpath = client.download_artifacts(run.info.run_id, ".")
# dbutils.fs.cp("file:" + outpath, "dbfs:/mnt/azureml/model", True)
